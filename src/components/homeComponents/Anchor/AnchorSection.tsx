...
import QuentinCard from './QuantinCard'

const AnchorSection: FC = () => {
  const [bigImg, setBigImg] = useState<0|1>(0)
  const [smallImg, setSmallImg] = useState<0|1>(0)
  ...
  const [switchingSmallImg, setSwitchingSmallImg] = useState<boolean>(false)
  ...
  const handleNext = () => {
    ...
    setTimeout(()=> {
      setSwitchingSmallImg(true)
    }, 600)

    setTimeout(()=> {
      setSmallImg(prev=> prev===1 ? 0 : 1)
      setSwitchingSmallImg(false)
    }, 850)
  ...
  const arraySmallImages = [
    "cabinet-filtered-2-small.jpg",
    "cabinet-lupi-2.png"
  ]
    ...
    <section className='w-[95%] desktop:w-[1440px] mx-auto mb-0 laptop:mb-72' style={{height: returnSectionHeight(height ?? 0, width?? 0)}}>
      <div className='w-full h-[660px] desktop:h-[700px] flex items-start relative'>
        ...
        <motion.article variants={opacities} custom={{delay: 1.8, duration: 0.25}} initial="initial" animate="animate" className="relative h-full w-1/2 hidden laptop:block">
          <Image src={`/images/${arrayBigImages[bigImg]}`} alt="cabinet chaise dentiste" style={{objectFit:"cover", transformOrigin:"top center"}} className='z-10' priority fill/>
        ...
        <article className='h-full w-full laptop:w-1/2 flex flex-col' >
          <motion.div variants={opacities} custom={{delay: 1.9, duration: 0.25}} initial="initial" animate="animate" className='relative h-1/2'>
            <Image src={`/images/${arraySmallImages[smallImg]}`} alt="Dentiste Sylvie Ma-Francin en intervention" style={{objectFit:"cover"}} priority fill/>
            <motion.div variants={reverseOpacity} custom={0} initial="initial" animate={switchingSmallImg ? "animate":"initial"} className='w-full h-full bg-white z-30 absolute' />
          </motion.div>
